    ...
    <div class="mx-4">
      ...
      <div class="space-y-4">
        ${product.pages.length > 0 || (session.bundles && session.bundles.some(b => b.products && b.products.some(bp => bp.productId === product.id)))
          ? `
            ${product.pages.map(page => renderPageCard(page)).join('')}
...
function renderPageCard(page) {
  // Fallback to default currency if page doesn't have one
  const currency = page.currency || DEFAULT_CURRENCY
      ...
      <div class="flex justify-between items-start">
        <div class="flex-1 min-w-0 mr-4">
          ...
          <div class="mt-1 space-y-1">
            <p class="muted-text">${t("pages.price")}: ${formatPrice(page.price, currency)}</p>
            <p class="muted-text">${t("pages.shipping")}: ${formatPrice(page.shippingPrice, currency)}</p>
            ${page.insurancePrice > 0 ? `<p class="muted-text">${t("pages.insurance")}: ${formatNumber(page.insurancePrice)} ${getCurrencySymbol(currency)}</p>` : ''}
    ...
    <div class="secondary-bg border border-default rounded-xl shadow-md p-4">
      <div class="flex justify-between items-start">
        <div class="flex-1 min-w-0 mr-4">
          ...
          <div class="mt-1 space-y-1">
            <p class="muted-text">${t("pages.price")}: ${formatPrice(bundle.price, currency)}</p>
            <p class="muted-text">${t("pages.shipping")}: ${formatPrice(bundle.shippingPrice, currency)}</p>
...
function formatPrice(price, currency) {
  if (price === undefined || price === null || price === "") return t("pages.na")
  try {
    return Number(price) === 0 ? t("pages.free") : `${formatNumber(price)} ${getCurrencySymbol(currency)}`
  } catch (e) {
