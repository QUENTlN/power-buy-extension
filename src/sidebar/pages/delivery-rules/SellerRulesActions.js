...
export function updateValueLabels(container, prefix, type, data) {
...
export function setupAddGroupButtonListener(session, seller, safeSellerId) {
    ...
    addGroupBtn.addEventListener('click', () => {
        ...
        const newGroupHtml = renderGroupItem(session, seller, { name: t("deliveryRules.newGroupPlaceholder") }, index, safeSellerId, getSellerProducts)
...
export function handleCalculationTypeChange(e) {
...
export function saveSellerRules(seller, safeSellerId, session) {
    ...
    if (copiedFromValue) {
        ...
        if (session.importFeesEnabled) {
            ...
            const customsCurrencySelect = document.querySelector('.customs-clearance-currency')
            if (customsFeeInput) currentRule.customsClearanceFee = parseFloat(customsFeeInput.value) || 0
            if (customsCurrencySelect) currentRule.customsFeeCurrency = customsCurrencySelect.value
        }
    }
